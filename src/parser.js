...
const grammar = () => build(`
      ...
      operation ->  "function" _ predicate _ "(" _ (letty _ ":" _):? expression:? _ ")" _ "{" _ expression _ "}" {% 
        ([func, ws1, operation, ws2, p1, ws3, letty, args, w4, p2, ws5, cb1, ws6, result]) => ["^", letty ? letty[0] : [], result, operation, args || []]
      %}
      ...
      quantifier -> "every" {% () => ["every"]  %}
                 ...
                 | "more-than" _ "(" _ [0-9]:+ _ ")" {% ([op, ws1, p1, ws2, number]) => [op, number.join("")] %}
                 | "fewer-than" _ "(" _ [0-9]:+ _ ")" {% ([op, ws1, p1, ws2, number]) => [op, number.join("")] %}
                 | "exactly" _ "(" _ [0-9]:+ _ ")" {% ([op, ws1, p1, ws2, number]) => [op, number.join("")] %}
